...
void landing::getDbList()
{
    ...
    qDebug() << "folderPath是: " << folderPath;

    if (!dbDir.exists())
    {
        QMessageBox::critical(this, tr("错误"), tr("文件夹找不到啦-landing.cpp"));
        return;
    }
...
void landing::on_logButton_clicked()
{
    ...
    if ((ui->userName->text() != "") && (ui->passWord->text() != "")) {
        ...
        qDebug() << "数据库名称: " << ui->dbName->currentText();
        qDebug() << "用户名: " << ui->userName->text();
        qDebug() << "密码: " << ui->passWord->text();
        // 检查登陆是否成功，成功后切换到主界面
